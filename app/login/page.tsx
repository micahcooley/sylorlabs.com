...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token && success && token !== processedToken) {
      // For OAuth login, we need to fetch user info
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      params.set('state', 'login'); // Tell callback we came from login
  ...
  return (
    <div className="min-h-screen bg-noir-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-20 left-20 w-96 h-96 bg-neon-cyan/5 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-20 right-20 w-96 h-96 bg-neon-magenta/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="neon-glass rounded-2xl p-8 animate-slide-up">
          ...
          <p className="text-gray-400 mt-2">Access your professional audio tools</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="relative">
              <input
                ...
                className="w-full bg-black/40 border border-neon-cyan/30 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-neon-cyan focus:shadow-neon-cyan transition-all duration-300"
                required
              />
            </div>
            ...
            <div className="text-center space-y-2">
              <div>
                <Link href="/login/password-reset" className="text-neon-cyan hover:text-neon-magenta transition-colors duration-200 text-sm">
                  Forgot your password?
                </Link>
              </div>
              <p className="text-gray-400 text-sm">
                Don't have an account?{' '}
                <Link href="/signup" className="text-neon-cyan hover:text-neon-magenta transition-colors duration-200">
                  Sign up
                </Link>
              </p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-neon-cyan to-neon-magenta text-white font-semibold py-3 rounded-lg hover:shadow-[0_0_15px_rgba(0,243,255,0.3)] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-[1.02] active:scale-[0.98]"
            >
          ...
          <p className="text-center text-gray-500 text-sm mt-6">
            Don&apos;t have an account?{' '}
            <Link
              href={`/signup${redirectUri ? `?redirect_uri=${encodeURIComponent(redirectUri)}` : ''}`}
              className="text-neon-magenta hover:text-neon-magenta/80 transition-colors font-medium"
            >
              Sign up
            </Link>
          </p>
